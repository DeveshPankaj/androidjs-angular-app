...
export class AppComponent {
  ...
  // socket: io = null;
  notification: Notification = new Notification(this.title, "Hello World", false);
  notify() {
...
class Notification {
	...
	constructor(title, msg, syncWithJs = false) {
		(<any>window).notificationCounter = (<any>window).notificationCounter || 1;
		this.data = { title, msg };
	...
	getRendomNo() {
		return (<any>window).notificationCounter++;
	}
	...
	showInMobile() {
		(<any>window).android.initNotification(this.title, this.msg);
		(<any>window).android.showNotification(this.id);
	}

	update({title, msg}: {title?: string, msg?: string}) {
